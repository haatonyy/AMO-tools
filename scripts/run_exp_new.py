...
def acquire_data(
    ...
    stage852: K10CR1 | None,
    stage3491: K10CR1 | None,
    csv_path: str,
    log_path: str,
    channels: list[int],
    i_init_mA: float,
    ...
    step_size_mA: float,
    step_delay_s: float,
    l2_position_deg_852: float | None,
    l2_position_deg_3491: float | None,
    ...
    # Move λ/2 stages if requested
    if l2_position_deg_852 is not None and stage852 is not None:
        data_io.log_event(log_path, f"Moving λ/2 stage852 to {l2_position_deg_852:.4f}°")
        print(f"[acquire] Moving λ/2 stage852 to {l2_position_deg_852:.4f}°")
        stage852.move_to(l2_position_deg_852)
        data_io.log_event(log_path, f"λ/2 stage852 reached {stage852.get_position():.4f}°")
    
    if l2_position_deg_3491 is not None and stage3491 is not None:
        data_io.log_event(log_path, f"Moving λ/2 stage3491 to {l2_position_deg_3491:.4f}°")
        print(f"[acquire] Moving λ/2 stage3491 to {l2_position_deg_3491:.4f}°")
        stage3491.move_to(l2_position_deg_3491)
        data_io.log_event(log_path, f"λ/2 stage3491 reached {stage3491.get_position():.4f}°")
...
def run_single_sweep(
    ...
    data_io.write_json_params(
        folder_name,
        {
            ...
            "pi852_mW":              pi852_mW,
            "pf852_mW":              pf852_mW,
            "l2_position_deg_852":   l2_position_deg_852,
            "l2_position_deg_3491":  l2_position_deg_3491,
    ...
    log_params = {
        ...
        "λ/2 position 852":       f"{l2_position_deg_852:.4f} deg" if l2_position_deg_852 is not None else "not set",
        "λ/2 position 3491":      f"{l2_position_deg_3491:.4f} deg" if l2_position_deg_3491 is not None else "not set",
    }
    for c in channels:
